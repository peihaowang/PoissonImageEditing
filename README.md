...
where Ω is the target region, ∂Ω denotes the boundaries between the source and target regions. *f\** is the given function defined on the entire domain (aka. the background), **v** represents its gradient vector, while *f* is defined as the unknown function on region Ω. The solution of the equation must satisfy Euler-Largrange equation:
